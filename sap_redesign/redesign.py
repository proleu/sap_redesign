...
def favor_native_residue_maker(sfxn: ScoreFunction, restraint: float
    ...
    xml_string = """
    <MOVERS>
        <FavorSequenceProfile name="favournative" weight="-100" use_current="true" matrix="IDENTITY"/>
...
def relax_script_maker(relax_script:str
    ...
    return script
# TODO documentation
def fast_relax_with_options(pose: Pose, repack_focus=[], flexbb=True,
                            relax_script="MonomerRelax2019") -> Pose:
    # sfxn calculations can be sped up by const_bb=True if not using PRO
    sfxn_hard = sfxn_hard_maker(const_bb=True)
    ...
    if len(repack_focus) == 0:
        focus_resis = list(get_residues_from_subset(true_sel.apply(pose)))
    else:
        focus_resis = repack_focus.copy()
    focus = ResidueIndexSelector()
    focus.set_index(','.join([str(x) for x in focus_resis]))
    packable = NeighborhoodResidueSelector()
    packable.set_focus_selector(focus)
    not_packable = NotResidueSelector()
    not_packable.set_residue_selector(packable)
    no_design = operation.RestrictToRepackingRLT()
    no_repack = operation.PreventRepackingRLT()
    pack = operation.OperateOnResidueSubset(no_design, true_sel)
    lock = operation.OperateOnResidueSubset(no_repack, not_packable)
    ...
    for op in [pack, lock, ic, arochi, ex1_ex2, prune]:
        task_factory.push_back(op)
    # setup movemap
    mm = pyrosetta.rosetta.core.kinematics.MoveMap()
    mm.set_bb(flexbb), mm.set_chi(True), mm.set_jump(False)
    # setup fast relax with sfxn_hard and relax_script
    fast_relax = FastRelax(scorefxn_in=sfxn_hard, standard_repeats=2,
                           script_file=relax_script)
    ...
    return pose
# TODO documentation, type check for pose
...
for file in files:
    if 'resurf_resurf' in file:
        path = os.path.join(root, file)
        pose = pyrosetta.io.pose_from_pdb(path)
        residue_sap_list = residue_sap_list_maker(pose)
        sorted_residue_sap_list = sorted(residue_sap_list, key=lambda x: x[1],
                                         reverse=True)
        worst_resis = [x[0] for x in sorted_residue_sap_list[:worst_n]]
        print(file, "worst residues by SAP", worst_resis)
        new_pose = fast_design_with_options(pose, to_design=worst_resis,
                                    cutoffs=(20, 40), flexbb=False,
                                    relax_script="MonomerDesign2019",
                                    restraint=0, use_dssp=True,
                                    use_sc_neighbors=False)
        new_pose.dump_pdb("{}_resurf.pdb".format(path.replace(".pdb",'')))
